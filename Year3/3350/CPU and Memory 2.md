...
$$\text{CPUTime} = \text{Instruction_Count} \times \text{CPI} \times \text{clock\_cycle}$$
...
$$  
CPUTime=IC \times (CPI_{ideal} +\text{Average memory stall cycles})\times CC  
$$
...
$$  
\text{Average mem stall cycle = access\_count }\times \text{miss rate }\times \text{miss\_penalty}  
$$
